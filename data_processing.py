...
def plot_cdf(continious_states_true, cdf_true, continious_states_rn, cdf_rn):
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(continious_states_true, cdf_true, color='skyblue', linewidth=2, label='Физическая CDF')
    plt.plot(continious_states_rn, cdf_rn, color='orange', linewidth=2, label='Риск-нейтральная CDF')
    plt.xlabel('Ожидаемая дох-ть')
...
def plot_volatility_forecast(realized_volatility_df, predictions, step=5):
    ...
    plt.plot(realized_volatility, label='Realized Volatility', color='skyblue', linewidth=2)
    
    # Добавляем предсказанные точки
    plt.scatter(future_indices, predictions, color='red', label='Predicted Volatility', zorder=10)
    ...
    plt.plot(x_values, y_values, color='orange', linestyle='-', linewidth=2, label='Forecast Line')
    
    # Обозначаем точки на графике
    for i, (index, pred) in enumerate(zip(future_indices, predictions), start=1):
        plt.text(index, pred, f't+{i * step}', fontsize=10, color='red', ha='right')
    
    # Добавляем подписи и легенду
    plt.title("Realized Volatility Forecast")
    plt.xlabel("Time")
