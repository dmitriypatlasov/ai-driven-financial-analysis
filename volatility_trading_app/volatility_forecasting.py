...
def plot_volatility_forecast(volatility_df, predictions, step=5):
    ...
    # Построение графика
    fig, ax1 = plt.subplots(figsize=(12, 6), facecolor='black')  # Создаем фигуру с черным фоном
    
    # График реализованной волатильности
    ax1.plot(realized_volatility, label='Realized Volatility', color='cyan', linewidth=2)
    
    # Добавляем предсказанные точки
    ax1.scatter(future_indices, predictions, color='orange', label='Predicted Volatility', zorder=10)
    ...
    ax1.plot(x_values, y_values, color='magenta', linestyle='-', linewidth=2, label='Forecast Line')
    
    # Обозначаем точки на графике
    for i, (index, pred) in enumerate(zip(future_indices, predictions), start=1):
        ax1.text(index, pred, f't+{i * step}', fontsize=10, color='white', ha='right')
    
    # Настройка первой оси
    ax1.set_title("Realized Volatility Forecast", color='white')
    ax1.set_xlabel("Time", color='white')
    ax1.set_ylabel("Realized Volatility", color='cyan')
    ax1.tick_params(axis='y', colors='cyan')
    ax1.tick_params(axis='x', colors='white')
    ax1.legend(loc='upper left', frameon=False, fontsize=10, facecolor='black', edgecolor='black')
    ax1.grid(False)
    
    # Создаем вторую ось для графика underlying_asset
    ax2 = ax1.twinx()  # Создание второй оси Y
    ax2.plot(underlying_asset.index, underlying_asset, label='Underlying Asset', color='lime', linewidth=1.5)
    
    # Настройка второй оси
    ax2.set_ylabel("Underlying Asset Price", color='lime')
    ax2.tick_params(axis='y', colors='lime')
    ax2.legend(loc='upper right', frameon=False, fontsize=10, facecolor='black', edgecolor='black')
    
    # Настройка фона графика и осей
