...
def plot_volatility_forecast(volatility_df, predictions, step=5):
    ...
    # Построение графика
    fig, ax1 = plt.subplots(figsize=(12, 6))  # Создаем фигуру и оси
    
    # График реализованной волатильности
    ax1.plot(realized_volatility, label='Realized Volatility', color='skyblue', linewidth=2)
    
    # Добавляем предсказанные точки
    ax1.scatter(future_indices, predictions, color='red', label='Predicted Volatility', zorder=10)
    ...
    ax1.plot(x_values, y_values, color='orange', linestyle='-', linewidth=2, label='Forecast Line')
    
    # Обозначаем точки на графике
    for i, (index, pred) in enumerate(zip(future_indices, predictions), start=1):
        ax1.text(index, pred, f't+{i * step}', fontsize=10, color='red', ha='right')
    
    # Настройка первой оси
    ax1.set_title("Realized Volatility Forecast")
    ax1.set_xlabel("Time")
    ax1.set_ylabel("Realized Volatility", color='skyblue')
    ...
    ax2.plot(underlying_asset.index, underlying_asset, label='Underlying Asset', color='green', linewidth=1.5)
    
    # Настройка второй оси
    ax2.set_ylabel("Underlying Asset Price", color='green')
